...
gulp.task("copy-assets", function() {
	...
	gulp.src(`${paths.node}bootstrap/scss/**/*.scss`).pipe(
	...
	gulp.src(
		`${paths.node}font-awesome/fonts/**/*.{ttf,woff,woff2,eot,svg}`
	).pipe(gulp.dest("./fonts"));

	// Copy all Font Awesome SCSS files
	gulp.src(`${paths.node}font-awesome/scss/*.scss`).pipe(
		gulp.dest(`${paths.dev}/sass/fontawesome`)
	);

	// _s SCSS files
	gulp.src(`${paths.node}undescores-for-npm/sass/media/*.scss`).pipe(
		gulp.dest(`${paths.dev}/sass/underscores`)
	);
	...
	// Tina Martina SCSS files
	gulp.src(`${paths.node}../understrap/sass/**/*.scss`).pipe(
		gulp.dest(`${paths.dev}/sass/understrap`)
	);

	return stream;
});

// Tina Martina translate files
		...
		gulp
			...
			.pipe(
				wppot({
					...
					team: "tinamartina <tinamartina@tinamartina.com>"
				})
			)
			.pipe(gulp.dest("languages/tina-martina.pot")),
		done()
	);
});
